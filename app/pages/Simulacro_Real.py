...
if not st.session_state.exam_active:
    ...
    # Load protocol data
    protocol_path = os.path.join(PROJECT_ROOT, "data", "protocolo_examen.json")
    try:
        import json
    ...
    if protocol:
        st.markdown(f"""
        <div style="background: linear-gradient(135deg, #E60000 0%, #8B0000 100%); color: white; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
            <h2 style="margin: 0; color: white;">📋 PROTOCOLO DEL DÍA DEL EXAMEN</h2>
            <p style="margin: 5px 0 0 0; opacity: 0.9;">Guía de Orientación al Aspirante (GOA) - CNSC Proceso {protocol['informacion_general']['proceso']}</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            st.markdown("### ⏰ Logística del Examen")
            log = protocol['logistica']
            st.info(f"""
            **Hora de llegada recomendada:** {log['hora_llegada_recomendada']}  
            **Cierre de puertas:** {log['hora_cierre_puertas']} ⚠️  
            **Excepciones:** {log['excepciones_cierre']}  
            **Duración total:** {log['duracion_total']}
            """)
        
        with col2:
            st.markdown("### ✏️ Material Permitido")
            mat = protocol['material_permitido']
            st.success("**Obligatorio:**\n" + "\n".join([f"- {item}" for item in mat['obligatorio']]))
            st.error("**Prohibido:**\n" + "\n".join([f"- {item}" for item in mat['prohibido'][:5]]))
        
        # Estructura del Examen
        st.markdown("### 📊 Estructura del Examen")
        est = protocol['estructura_examen']
        
        col_f, col_c, col_i = st.columns(3)
        
        with col_f:
            func = est['distribucion']['funcional']
            st.markdown(f"""
            <div style="background: #2ecc71; color: white; padding: 15px; border-radius: 10px; text-align: center;">
                <h3 style="margin: 0; color: white;">{func['porcentaje']}%</h3>
                <p style="margin: 5px 0 0 0;">FUNCIONAL</p>
                <small>~{func['cantidad_aproximada']} preguntas</small><br>
                <small>⚠️ ELIMINATORIO (mín. {func['puntaje_minimo_aprobacion']}%)</small>
            </div>
            """, unsafe_allow_html=True)
        
        with col_c:
            comp = est['distribucion']['comportamental']
            st.markdown(f"""
            <div style="background: #3498db; color: white; padding: 15px; border-radius: 10px; text-align: center;">
                <h3 style="margin: 0; color: white;">{comp['porcentaje']}%</h3>
                <p style="margin: 5px 0 0 0;">COMPORTAMENTAL</p>
                <small>~{comp['cantidad_aproximada']} preguntas</small>
            </div>
            """, unsafe_allow_html=True)
        
        with col_i:
            integ = est['distribucion']['integridad']
            st.markdown(f"""
            <div style="background: #9b59b6; color: white; padding: 15px; border-radius: 10px; text-align: center;">
                <h3 style="margin: 0; color: white;">{integ['porcentaje']}%</h3>
                <p style="margin: 5px 0 0 0;">INTEGRIDAD</p>
                <small>~{integ['cantidad_aproximada']} preguntas</small><br>
                <small>Escala Likert</small>
            </div>
            """, unsafe_allow_html=True)
        
        # Alertas importantes
        st.markdown("### ⚠️ Alertas Críticas")
        for alerta in protocol['alertas_importantes']:
            st.warning(alerta)
