...
def load_exam_cases():
    """Carga 'casos' simulados agrupando preguntas por tema"""
    db = next(get_db())
    user_id = st.session_state.get("user_id")

    # Simular casos agrupando preguntas por tema
    class SimulatedCase:
    ...
    try:
        ...
        topics = db.query(Question.topic).distinct().limit(10).all()
        topics = [t[0] for t in topics if t[0]]
        
        if not topics:
            return []
        
        # Seleccionar 3 temas aleatorios
        import random
        selected_topics = random.sample(topics, min(3, len(topics)))
        
        for topic in selected_topics:
            # Obtener 3-5 preguntas del tema
            ...
            if questions:
                case = SimulatedCase(
                    title=f"Caso: {topic}",
                    text=f"Este caso evalúa tu conocimiento en {topic}. Lee cuidadosamente cada pregunta y selecciona la mejor respuesta basándote en tus conocimientos sobre este tema.",
                    questions=questions
                )
...
def finish_exam():
    ...
    for case in st.session_state.exam_cases:
        for q in case.questions:
            ...
            # Save Attempt to DB (only if StatsService is available)
            if user_id and StatsService:
                try:
